...
class BookingRequestRepo {
  ...
  Future<Response> getBookingCalendarData({
    ...
    Map<String, dynamic> params = {'mode': mode};
    ...
    if (bookingStatus != null && bookingStatus.isNotEmpty) {
      final statusParams = bookingStatus
          .map((status) => 'booking_status[]=${Uri.encodeComponent(status)}')
          .join('&');
      queryString = queryString.isEmpty
          ? statusParams
          : '$queryString&$statusParams';
    }

    return await apiClient.getData(
      '${AppConstants.bookingCalenderList}?$queryString',
    );
  }
